...
class Usuarios extends Controllers
{
    ...
    public function getusuarios()
    {
        ...
        for ($i = 0; $i < count($arrdata); $i++) {
            ...
            // --- PERMISOS ---
            if (!empty($_SESSION['permisosmod']['r'])) {
                $btnview = '<a onClick="fntviewcliente(' . $arrdata[$i]['idusuario'] . ')" class="dropdown-item">Detalles</a>';
            }

            if (!empty($_SESSION['permisosmod']['u'])) {
                $btnedit = '<a class="dropdown-item btneditusuario" rl="' . $arrdata[$i]['idusuario'] . '">Editar</a>';
            }
            ...
            // Si es el último, agregar script para que funcionen los botones
            if ($i == (count($arrdata) - 1)) {
                $script = '<script type="text/javascript"> fnteditusuario(); fntdelusuario();</script>';
            }

            // Construcción del menú según permisos
