...
class Categorias extends Controllers
{
    ...
    public function getcategorias()
    {
        ...
        for ($i = 0; $i < count($arrdata); $i++) {
            ...
            // --- PERMISOS ---
            if (!empty($_SESSION['permisosmod']['r'])) {
                // Si luego deseas, puedes habilitar algo como detalles aquí:
                // $btnview = '<a onClick="fntviewcategoria(' . $arrdata[$i]['idcategoria'] . ')" class="dropdown-item">Detalles</a>';
            }

            if (!empty($_SESSION['permisosmod']['u'])) {
                $btnedit = '<a class="dropdown-item btneditcategorias" rl="' . $arrdata[$i]['idcategoria'] . '">Editar</a>';
            }
            ...
            <a data-toggle="dropdown" data-caret="false" class="text-muted" aria-expanded="false">
